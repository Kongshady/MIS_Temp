    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'add') {
            ...
            if ($check_client->num_rows == 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO transaction (client_id, or_number, datetime_added, status_code) VALUES (?, ?, NOW(), ?)");
                $stmt->bind_param("isi", $client_id, $or_number, $status_code);
            ...
            if ($check_client->num_rows == 0) {
                ...
                $stmt = $conn->prepare("UPDATE transaction SET client_id=?, or_number=?, status_code=? WHERE transaction_id=?");
                $stmt->bind_param("isii", $client_id, $or_number, $status_code, $transaction_id);
...
$transactions = $conn->query("SELECT t.*, 
                             CONCAT(COALESCE(p.firstname, ''), ' ', COALESCE(p.lastname, '')) as client_name,
                             s.label as status_label 
                             FROM transaction t 
                             LEFT JOIN patient p ON t.client_id = p.patient_id 
                             LEFT JOIN status_code s ON t.status_code = s.status_code 
                             ORDER BY t.datetime_added DESC");

// Check if there are any patients
$patient_count = $conn->query("SELECT COUNT(*) as count FROM patient")->fetch_assoc()['count'];
$status_count = $conn->query("SELECT COUNT(*) as count FROM status_code")->fetch_assoc()['count'];
?>
...
<div class="container">
    ...
    <?php if ($status_count == 0): ?>
        <div class="alert alert-warning">
            <strong>⚠️ No Status Codes Found!</strong> You need to add status codes to the database before you can create transactions.
        </div>
    <?php endif; ?>
    ...
    <div class="card">
        ...
        <?php if ($patient_count > 0 && $status_count > 0): ?>
        ...
        <div class="alert alert-info">
            ...
            <ul>
                <li><a href="patients.php">Patients in the system</a></li>
                <li>Status codes configured in the database</li>
            </ul>
        </div>
        <?php endif; ?>
                ...
                <tbody>
                    <?php while($transaction = $transactions->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($transaction['status_label']); ?></td>
...
function editTransaction(transaction) {
    ...
    document.getElementById('edit_status_code').value = transaction.status_code;
